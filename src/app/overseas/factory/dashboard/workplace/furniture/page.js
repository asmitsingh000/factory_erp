...
export default function FurniturePage() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      setProductionData([
        { id: 1, particulars: "Furniture Core Operations Ledger", quantity: "4,850 Inventory Units", timestamp: "Synced - June 2026" }
      ]);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.itemName || !formData.quantity) return alert("Fields are required!");
    ...
    try {
      ...
      alert(`Database Success: Furniture log ${action === 'edit' ? 'Updated' : 'Created'}!`);
      closeModal();
    } catch (err) {
    ...
    <div className="space-y-4 animate-in fade-in duration-200">
      <div className="bg-[#0b0f19] border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-left text-sm border-collapse">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="5" className="p-4 text-center text-gray-500">Loading factory database...</td></tr>
            ) : (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#0b0f19] border border-gray-800 w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-[#080b13]">
              <h2 className="text-md font-bold text-white">{action === 'edit' ? '🛠️ Edit Furniture Log' : '🪑 Log Furniture Production'}</h2>
              <button onClick={closeModal} className="text-gray-400 hover:text-white">✕</button>
            </div>
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase">Furniture Model Name</label>
                <input type="text" placeholder="e.g., Luxury Sofa, Dining Table" value={formData.itemName} onChange={(e) => setFormData({ ...formData, itemName: e.target.value })} className="w-full bg-[#141b2d] border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500" />
              </div>
              <div>
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={closeModal} className="flex-1 py-2.5 rounded-lg border border-gray-700 text-gray-300 text-sm hover:bg-gray-800">Cancel</button>
