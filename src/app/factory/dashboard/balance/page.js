//this is the page for the balance it is a frontend file its route is app/factory/dashboard/balance/page.js
...
function BalanceContent() {
  ...
  const fetchLedger = async () => {
    try {
      ...
      setLedgerData([
        { id: 1, date: "2026-06-09", particulars: "Opening Balance Transfer", type: "Credit", amount: 50000, balance: 450000 }
      ]);
    } catch (err) {
  ...
  return (
    <div className="flex-1 flex flex-col space-y-6 animate-fade-in w-full text-white">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[#0b0f19] p-4 rounded-xl border border-gray-800 gap-4">
        <div>
          <h1 className="text-4xl font-bold text-whit e">Balance Ledger Sheet</h1>
        ...
        <div className='gap 23'>
          ...
          {!isReadOnly && (
            <button
              ...
              className="bg-[#155DFC] hover:bg-[#4177ed] text-white text-xs px-4 py-2.5 rounded-lg font-bold transition shadow-lg whitespace-nowrap"
            >
              + Add Ledger Entry
            </button>
          )}
      ...
      <div className="bg-[#0b0f19] border border-gray-800 rounded-xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left text-sm min-w-[600px]">
            <thead className="bg-[#532805] text-white-400 font-semibold border-b border-gray-800 text-2xl">
              <tr>
            ...
            <tbody className="text-gray-200 divide-y divide-gray-800">
              {loading ? (
                ...
                ledgerData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-800/20 transition text-xl">
                    <td className="p-4  font-mono text-white-400 font-semibold ">{item.date}</td>
